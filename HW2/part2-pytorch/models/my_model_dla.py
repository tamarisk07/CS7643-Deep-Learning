...
class Root(nn.Module):
    ...
    def forward(self, x):
        out = torch.cat(x, 1)
        out = F.relu(self.batch(self.conv(out)))
        return out
...
class MyModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out_l1 = self.layer1(out_base)
        out_l2 = self.layer2(out_l1)
        out_l3 = self.layer3(out_l2)
        out_l4 = self.layer4(out_l3)
        out_l5 = self.layer5(out_l4)
        out_l6 = self.layer6(out_l5)
        out_pool = F.avg_pool2d(out_l6, 4)
        out_fc = out_pool.view(-1, 512)
        out = self.linear(out_fc)
        return out